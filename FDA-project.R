...
id <- c(c(1:24), c(66:81))[-c(20,35)] #missing ids
...
#CONFUSION MATRIX
table(scores$group, kmeans_c1$cluster)

#mean(scores$group == kmeans_c1$cluster)
fviz_cluster(kmeans_c1, data = features_x, 
             main = "K-Means Clusters")
#==============================================================================
#plot original data coloured by cluster
#facet_wrap() schizophrenia/gender...

##GLM logistic regression Schizophrenia = outcome, FPC1,2,3 = predictors,
#RMD report
#save results of long portions as a .R data file e.g. reading in the data
#save(resMean, file = "FPCAC1T1.Rdata")
#Full analysis T1 C1 rmd
