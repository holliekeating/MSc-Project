...
data, whereas high bias may over-smooth the data leading to underfitting
and obscuring important signal characteristics.

Spline smoothing uses piecewise polynomial functions joined smoothly
...
smoothing estimates a curve \(f(t_i)\) from our observations assuming
\(y_i = f(t_i) + \epsilon_i\) where \(t_i\) represents the time points
along the x-axis of the curve, \(f(t_i)\) represents the true underlying
curve and \(\epsilon_i ~ iid(0, \sigma^2)\) represents the exogenous
error, which contributes to the roughness of the raw data.

We estimate \(f(t)\) by minimising the following:

\[\Sigma_{i=1}^n (y_i – f(t_i))^2 + \lambda\int [f’’(t)]^2 dt\]

Where \(\Sigma_{i=1}^n (y_i – f(t_i))^2\) is the residual sum of square
(RSS) and \(\int [f’’(t)]^2 dt\) is a penalty that prevents excessive
wiggliness by preventing it from changing slope too quickly (as
\(f’’(t)\) measures the rate of change of the slope) and \(\lambda\) is
the smoothing parameter where \(\lambda = 0\) is the \(RSS\) fit which
is excessively wiggly and \(\lambda \rightarrow \inf\) is oversmoothed
and underfit.

We estimate \(f(t)\) by assuming that
\(f(t) = \Sigma_{k=1}^K c_k B_k(t)\) where \(B_k(t)\) are the cubic
basis functions that we know, \(K =100\) is the total number of knots
that have been selected which are equally spaced along the x-axis and
\(c_k\) are the coefficients of the cubic b-splines we are estimating.

**insert plot of raw data and smoothed means

\begin{figure}[H]

{\centering \includegraphics[width=0.8\linewidth]{Images/raw_smooth_data} 
...
{\centering \includegraphics[width=0.8\linewidth]{Images/FPCA} 
...
{\centering \includegraphics[width=0.8\linewidth]{Images/FPC1_top_bot} 

}

\caption{Curves with highest 5 FPC 1 scores (blue) and lowest 5 scores (red)}\label{fig:unnamed-chunk-5}
\end{figure}

Those with the highest scores for the first principal component (in
blue) remain close to the mean at the beginning and go down over time,
and those with the lowest scores for the first principal component (in
red) remain close to the mean and then rise up above the mean over time.
The top scorers following a similar trajectory to that of the first
eigenfunction shown above and the lowest scorers following an inverse
trajectory.

\begin{figure}[H]

{\centering \includegraphics[width=0.7\linewidth]{Images/FPC2_top_bot} 

}

\caption{Curves with highest 5 FPC 2 scores (blue) and lowest 5 scores (red)}\label{fig:unnamed-chunk-6}
\end{figure}

\begin{figure}[H]

{\centering \includegraphics[width=0.7\linewidth]{Images/FPC3_top_bot} 

}

\caption{Curves with highest 5 FPC 3 scores (blue) and lowest 5 scores (red)}\label{fig:unnamed-chunk-7}
\end{figure}
...
{\centering \includegraphics[width=0.7\linewidth]{Images/fpca_kmeans_plot} 

}

\caption{Visulisation of K-means clustering of FPC scores}\label{fig:unnamed-chunk-8}
\end{figure}

\begin{center}\includegraphics[width=0.12\linewidth]{Images/FPCA_km_cm} \end{center}

\begin{figure}[H]

{\centering \includegraphics[width=0.7\linewidth]{Images/functions_by_cluster} 

}

\caption{Visulisation of functions in each of the K-means clusters}\label{fig:unnamed-chunk-10}
\end{figure}

\subsection{Elastic Net}\label{elastic-net-1}

An elastic net model was trained using the training dataset and
subsequently evaluated on the independent test dataset. The model
incorporated data from all three experimental conditions, four EEG nodes
(fp1, f3, f7 and af7), and mfpca scores explaining 95\% of the
variability at both levels 1 and 2. Using 10-fold cross-validation, the
optimal tuning parameters were identified as \(\alpha = 0.71\) and
\(\lambda = 0.000081\). The original set of 290 predictor variables was
reduced to 35 non-zero coefficients (Figure a), indicating substantial
variable selection. Model discrimination was high, with an AUC of 93.7\%
(Figure b). The ROC curve demonstrated strong separation between cases
and controls across a range of classification thresholds. Overall
classification accuracy on the test set was 74.2\%. Examination of the
calibration plot (Figure c) indicated imperfect calibration, with a
calibration slope ranging from 0.47 to 0.70. This pattern was also
evident in the distribution of predicted probabilities (Figure d), where
predictions tended to cluster near 0 and 1 rather than being spread
across the full probability range. Although the model demonstrated
strong discriminatory performance, the calibration results suggest that
the model was overconfident in its predictions, producing probabilities
that were more extreme than those observed in the data.
...
\caption{Model performance results for all conditions: (a) Coefficient Plot; (b) ROC Curve; (c) Calibration Plot; (d) Predicted Probability Plot.}\label{fig:unnamed-chunk-11}
\end{figure}
